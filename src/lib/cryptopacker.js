module.exports = (function () {
  ...
  function _getPicamoSection(file, callback) {
    var result = {
        ...
        bufferPos = 0,
        detectBuffer = Buffer.from(jpegEOF.concat(marker),'hex'),
  ...
  function _getKeyAndIV(key, callback) {
    crypto.pseudoRandomBytes(ivSize, function (err, ivBuffer) {
  ...
  function _getEncryption(camoOptions, callback) {
    ...
    // no paranoia
      ...
      //IV is filled with zeroes
      encryption.cipher = crypto.createCipheriv(
        algorithm, 
        crypto.createHash(hashFunc).update(camoOptions.password).digest('base64').substr(0, keySize), 
        "0".repeat(ivSize)
      );
      callback(encryption);
    }
        ...
        if (err)
          ...
          try {
            ...
            decryption.cipher = crypto.createDecipheriv(
              algorithm,
              Buffer.from(keyIV.key,'hex'),
              Buffer.from(keyIV.iv, 'hex')
            );
  ...
  function _buildHideStream(camoOptions, callback) {
      ...
      */
      var combinedStream = CombinedStream.create(),
          ...
          zip = zlib.createGzip(),
          illusioSignature = Buffer.from(marker, "hex"),
          hiddenFile = fs.createReadStream(camoOptions.file),
          fileNameSize = Buffer.alloc(2),
  ...
  function _buildShowStream(camoOptions, header, callback) {
    ...
    */
    var picamoHeader = Buffer.from(jpegEOF.concat(marker),'hex'),
