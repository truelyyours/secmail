...
/*  hide: boolean, true means to hide `message`
    message: string, the message to be hidden, if any
    autoimage: string, , seperated tag,widthxheight (in pixels)
    showimage: string, image data as hex only passed when hide is false
    passwd: string, passed in both cases
    hat: string, the hat file generated while encrypting and hiding.
    */

function passArguments(hide, message=null,autoimage=null,showimage=null, passwd, hat=null) {
  ...
  if (hide) {
    ...
    // if (argv.autoimage !== undefined) {
        var result = worker.parseAutoImage(autoimage);
  ...
  if (!hide) {
    ...
    // else
    if (fileExists(showimage))
      options.image = showimage;
    else
...
//   options.compress = (argv.nocomp === undefined);
  ...
  //check for custom marker ## NO MARKER FOR NOW, CAN BE DSA #### 
